...
export default function Progress() {
  ...
  const alphabetProgress = Array.from({ length: 26 }, (_, i) => {
    ...
    const totalWords = spellingWords.filter(word =>
      word.charAt(0).toUpperCase() === letter
    ).length;

    const practicedWords = stats.wordsLearned.filter(word =>
      word.charAt(0).toUpperCase() === letter
    ).length;
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-7xl">
        ...
        <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-lg rounded-2xl shadow-lg border border-white/20 dark:border-slate-700/50 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 dark:text-white">{stats.wordsLearned.length}</p>
              <p className="text-xs text-gray-500 dark:text-gray-500">of {stats.totalWords}</p>
            </div>
        ...
        <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-lg rounded-2xl shadow-lg border border-white/20 dark:border-slate-700/50 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 dark:text-white">{stats.averageAccuracy}%</p>
            </div>
        ...
        <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-lg rounded-2xl shadow-lg border border-white/20 dark:border-slate-700/50 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 dark:text-white">{stats.totalPracticeSessions}</p>
      ...
      <div className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {achievements.map((achievement, index) => (
            <div
              key={index}
              className={`bg-white/70 dark:bg-slate-900/70 backdrop-blur-lg rounded-2xl shadow-lg border border-white/20 dark:border-slate-700/50 p-6 ${achievement.achieved ? 'ring-2 ring-green-500 dark:ring-green-400' : ''
                }`}
            >
              <div className="flex items-center space-x-4 mb-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${achievement.achieved
                ...
                <div className="flex-1">
                  <h3 className={`font-bold ${achievement.achieved
      ...
      {/* Alphabet Progress */}
      <div className="mb-12">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Progress by Letter</h2>
        <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-lg rounded-2xl shadow-lg border border-white/20 dark:border-slate-700/50 p-6">
          <div className="grid grid-cols-6 md:grid-cols-13 gap-4">
            {alphabetProgress.map(({ letter, totalWords, practicedWords, progress }) => (
              <div
                key={letter}
      ...
      {stats.lastPracticeDate && (
        <div className="text-center mt-8">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Last practice session: {new Date(stats.lastPracticeDate).toLocaleDateString()}
          </p>
        </div>
      )}
    </div>
